...
public class ElectricDecorationTest {
    ...
    void testGetAndSetFields() {
        samples.forEach((a)->{
            ...
            Assertions.assertEquals(fields.get(5).getValue().toString(), a.getColorsOfLights().toString());
    ...
    void testToString(){
        samples.forEach((a) -> {
            ...
            Assertions.assertTrue(tmp.contains(a.getColorsOfLights().toString()), String.format("Error in colors of lights %d dec", samples.indexOf(a)));
            Assertions.assertTrue(tmp.contains(String.valueOf(a.getLength())), String.format("Error in length %d dec", samples.indexOf(a)));
            Assertions.assertTrue(tmp.contains(String.valueOf(a.getPowerInWatts())), String.format("Error in power %d dec", samples.indexOf(a)));
            Assertions.assertTrue(tmp.contains(String.valueOf(a.getAmountLampsPerMeter())), String.format("Error in amount per meter %d dec", samples.indexOf(a)));
