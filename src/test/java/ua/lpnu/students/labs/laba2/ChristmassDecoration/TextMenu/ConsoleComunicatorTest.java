...
public class ConsoleComunicatorTest {
    ...
    void testEdit() throws ParseException {
        ...
        List<List<String>> previousStringList = Arrays.asList(
            new ArrayList<>(Arrays.asList(new String("bla"))), new ArrayList<>(Arrays.asList("super", "puper", "test")), new ArrayList<>(Arrays.asList("saj", "test")));
        List<List<String>> resultStringList = Arrays.asList(
                Arrays.asList("bla", "bla", "bal"), Arrays.asList("super", "puper", "test"), Arrays.asList("test"));
        ...
        String[] inputTypeList = {
                ...
                List<List<Type>> previousTypeList = Arrays.asList(
                    Arrays.asList(Type.ELECTRIC_DECORATION), Arrays.asList(Type.PIECE_DECORATION, Type.ORGANIC_DECORATION, Type.LONG_DECORATION), Arrays.asList(Type.LONG_DECORATION),
                    Arrays.asList( Type.ORGANIC_DECORATION));
        List<List<Type>> resultTypeList = Arrays.asList(
                Arrays.asList(Type.PIECE_DECORATION), Arrays.asList(Type.PIECE_DECORATION, Type.ORGANIC_DECORATION, Type.LONG_DECORATION), Arrays.asList(Type.ORGANIC_DECORATION),
        ...
        String[] inputUsageList = {
                ...
                List<List<Usage>> previousUsageList = Arrays.asList(
                    Arrays.asList(Usage.FOR_WINDOWS), Arrays.asList(Usage.OUTSIDE_DECORATION, Usage.FOR_WINDOWS),
        ...
        List<List<Usage>> resultUsageList = Arrays.asList(
                Arrays.asList(Usage.UNKNOWN, Usage.FOR_WINDOWS), Arrays.asList(Usage.OUTSIDE_DECORATION, Usage.FOR_WINDOWS),
    ...
    Date[] previousDate = {
        ...
        for (int i = 0; i < inputUsage.length; i++) {
            ...
            var console = new ConsoleComunicator();
            Assertions.assertEquals(console.edit(previousDate[i], ""),
                    resultUsage[i]);
        }
    ...
    void testSet() throws ParseException {
        ...
        String[] inputTypeList = {
                "0\n5\n4\n\n", "\n", "1\n\n", "0\n1\n2\n\n" };
        List<List<Type>> resultTypeList = Arrays.asList(
                Arrays.asList(Type.PIECE_DECORATION), new LinkedList<>(), Arrays.asList(Type.ORGANIC_DECORATION),
                Arrays.asList(Type.PIECE_DECORATION, Type.ORGANIC_DECORATION, Type.LONG_DECORATION));
        for (int i = 0; i < inputTypeList.length; i++) {
            ByteArrayInputStream buf = new ByteArrayInputStream(inputTypeList[i].getBytes());
            System.setIn(buf);
            ...
            var console = new ConsoleComunicator();
            Assertions.assertEquals(console.set(new ArrayList<>(Arrays.asList(Type.ELECTRIC_DECORATION)), ""),
                    resultTypeList.get(i));
        }
