...
public class OrganicDecorationTest {
    ...
    void testGetAndSetFields() {
        samples.forEach((a)->{
            ...
            Assertions.assertEquals(fields.get(9).getValue(), new GregorianCalendar(2009, 9, 29).getTime());
        });
    }
    ...
    void testToString(){
        samples.forEach((a) -> {
            ...
            Assertions.assertTrue(tmp.contains(a.getColor().toString()), String.format("Error in colors of lights %d dec", samples.indexOf(a)));
            Assertions.assertTrue(tmp.contains(a.getStyle()), String.format("Error in length %d dec", samples.indexOf(a)));
